...
function filterChatActionsWithUndefinedValue(chatActions: ChatLiveActionWithVideoOffsetTime[]): ChatLiveActionWithVideoOffsetTime[] {
    return chatActions
        .filter(action => action.addChatItemAction && action.addChatItemAction.item)
        .filter(
            ({ addChatItemAction }) =>
...
function createAdvanceChatLiveActions(chatActions: ChatLiveActionWithVideoOffsetTime[]): AdvancedChatLiveActions {
    return chatActions.map(
        (action): AdvancedChatLiveAction => ({
            ...action,
            uuid: uuidV4(),
        })
    )
...
export const LiveChatResponse2LiveChatActions = (response: LiveChatResponse): AdvancedChatLiveActions => {
    ...
    const actions = [...((FindObjectByKeyRecursively(response as Response, 'actions') as YTLiveChat.LiveAction[]) || [])].map((action, i, arr) =>
        Object.assign(action, { videoOffsetTimeMsec: currentPlayerTime + i * (timeUntilNextRequest / arr.length) })
    )
...
export const LiveChatResponse2LiveChatReplayActions = (response: LiveChatResponse): AdvancedChatLiveActions => {
    ...
    const actions = replayActions
        ...
        .map(({ actions: liveActions, videoOffsetTimeMsec }) => {
            return { ...liveActions[0], videoOffsetTimeMsec: parseFloat(videoOffsetTimeMsec) || 0 }
        })
...
export const FilterDuplicateChatAdvancedChatLiveActions = (actions: AdvancedChatLiveActions): AdvancedChatLiveActions => {
    ...
    return actions.filter(({ addChatItemAction }) => {
        ...
        if (item.liveChatTextMessageRenderer) id = item.liveChatTextMessageRenderer.id
        else if (item.liveChatPaidMessageRenderer) id = item.liveChatPaidMessageRenderer.id
        else if (item.liveChatMembershipItemRenderer) id = item.liveChatMembershipItemRenderer.id
        if (!id) return false
...
export const getChatIframe = () => {
    const iframe = Array.from(document.getElementsByTagName('iframe')).find(i => i.classList.contains('ytd-live-chat-frame'))
...
export const getChatIframeScript = () => {
    ...
    if (!iframe || !iframe.contentDocument) return undefined
    return Array.from(iframe.contentDocument.getElementsByTagName('script')).find(i => i.innerText.includes('ytInitialData'))
}
...
export const parseInitLiveChatResponseFromScript = (scriptContent: string) => {
    ...
    if (dataString[dataString.length - 1] === ';') dataString = dataString.slice(0, dataString.lastIndexOf(';'))
    return dataString
}
