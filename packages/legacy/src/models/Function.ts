...
export const FillWithPresetValueWhenNotExist = <T extends Record<string, unknown>, U extends keyof T>(oldValue: T, newValue: T): T => {
    ...
    for (const key of keys) {
        if (target[key] === undefined) target[key] = newValue[key]
            ...
            target[key] = FillWithPresetValueWhenNotExist(target[key], newValue[key] as any) as T[U]
        }
    }
    return target
}
