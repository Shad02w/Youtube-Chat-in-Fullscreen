    ...
    return (
        ...
        badges.some(
            badge =>
                ...
                badge.liveChatAuthorBadgeRenderer.icon &&
                badge.liveChatAuthorBadgeRenderer.icon.iconType &&
                badge.liveChatAuthorBadgeRenderer.icon.iconType === type
        )
    )
}

export const isMember = (badges: YTLiveChat.AuthorBadge[] | undefined) => {
    return badges && badges.some(badge => badge.liveChatAuthorBadgeRenderer.customThumbnail)
}
...
export const getChatType = (action: AdvancedChatLiveAction): keyof ChatFilter => {
    ...
    if (liveChatTextMessageRenderer && checkBadgeType(liveChatTextMessageRenderer.authorBadges, 'OWNER')) return 'owner'
    else if (liveChatTextMessageRenderer && checkBadgeType(liveChatTextMessageRenderer.authorBadges, 'MODERATOR')) return 'moderator'
    else if (liveChatTextMessageRenderer && isMember(liveChatTextMessageRenderer.authorBadges)) return 'member'
    else if (liveChatPaidMessageRenderer) return 'superchat'
    else if (liveChatMembershipItemRenderer) return 'membership'
    return 'guest'
}
